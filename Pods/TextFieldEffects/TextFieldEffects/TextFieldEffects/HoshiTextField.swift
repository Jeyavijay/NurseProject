...
@IBDesignable open class HoshiTextField: TextFieldEffects {
    ...
    */
    ...
    private func rectForBorder(_ thickness: CGFloat, isFilled: Bool) -> CGRect {
        if isFilled {
            return CGRect(origin: CGPoint(x: 0, y: frame.height-1), size: CGSize(width: frame.width, height: thickness))
        } else {
            return CGRect(origin: CGPoint(x: 0, y: frame.height-1), size: CGSize(width: 0, height: thickness))
        }
    }
